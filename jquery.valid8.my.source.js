		...
		valid8: function (options) {
			...
			return this.each(function () {
				...
				var defaultOptions = {
					...
					values: null,
					defaultErrorMessage: '必須項目です。'
				};

				options = options || {};
				if (typeof options == 'string')	options = {defaultErrorMessage: options}

				// extendOptions のオプションを $.valid8 に上書き保存。
				$.valid8 = options.extendOptions || $.valid8;
				// 保存したオプションを優先してマージする
				$(this).data('settings', $.extend( $.extend(defaultOptions, options), $.valid8));
